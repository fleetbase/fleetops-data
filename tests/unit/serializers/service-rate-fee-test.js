...
module('Unit | Serializer | service rate fee', function (hooks) {
    setupTest(hooks);

    hooks.beforeEach(function () {
        this.store = this.owner.lookup('service:store');
        this.serializer = this.store.serializerFor('service-rate-fee');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'service-rate-fee');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'service-rate-fee', {
    ...
    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'service-rate-fee', { label: 'contract-value' });

        assert.strictEqual(record.label, 'contract-value', 'the attribute survives normalization');
    });

    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('service-rate-fee', { label: 'contract-value' });

        assert.strictEqual(record.serialize().label, 'contract-value');
    });
});
