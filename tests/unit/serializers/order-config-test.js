...
module('Unit | Serializer | order config', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        this.serializer = this.store.serializerFor('order-config');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'order-config');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'order-config', {});
    });

    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'order-config', { name: 'contract-value' });
    ...
    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('order-config', { name: 'contract-value' });
