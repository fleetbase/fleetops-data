...
module('Unit | Serializer | asset', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        this.serializer = this.store.serializerFor('asset');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'asset');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'asset', {
    ...
    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'asset', { name: 'contract-value' });
    ...
    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('asset', { name: 'contract-value' });

        assert.strictEqual(record.serialize().name, 'contract-value');
    });

    test('the vendor relationship is linked by uuid when it is not embedded', function (assert) {
        const related = this.store.push(this.store.normalize('vendor', { uuid: 'related_1' }));
        const record = this.store.createRecord('asset');
        record.set('vendor', related);

        assert.strictEqual(record.serialize().vendor_uuid, 'related_1', 'the application serializer always adds the identifier');
    });

    test('an unset vendor contributes no identifier', function (assert) {
        const json = this.store.createRecord('asset').serialize();

        assert.notOk(json.vendor_uuid, 'a relationship that was never set is simply absent');
    });
});
