...
module('Unit | Serializer | customer', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        this.serializer = this.store.serializerFor('customer');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'customer');
    });
    ...
    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'customer', { name: 'contract-value' });
    ...
    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('customer', { name: 'contract-value' });
