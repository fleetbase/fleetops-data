...
module('Unit | Serializer | manifest', function (hooks) {
    setupTest(hooks);

    hooks.beforeEach(function () {
        this.store = this.owner.lookup('service:store');
        this.serializer = this.store.serializerFor('manifest');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'manifest');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'manifest', {
    ...
    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'manifest', { status: 'contract-value' });
    ...
    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('manifest', { status: 'contract-value' });

        assert.strictEqual(record.serialize().status, 'contract-value');
    });

    test('the driver relationship travels inline with the record', function (assert) {
        const record = this.store.createRecord('manifest');
        record.set('driver', this.store.createRecord('driver', { name: 'related-value' }));

        const json = record.serialize();

        assert.strictEqual(json.driver.name, 'related-value', 'the related record is sent whole, not as an identifier');
    });

    test('an embedded payload normalizes into a linked record', function (assert) {
        const record = this.store.push(
            this.store.normalize('manifest', {
                uuid: 'manifest_1',
                driver: { uuid: 'related_1', name: 'related-value' },
            })
        );

        assert.strictEqual(record.belongsTo('driver').id(), 'related_1', 'the inline record becomes a real related record');
        assert.strictEqual(this.store.peekRecord('driver', 'related_1').name, 'related-value', 'and it carries the embedded attributes');
    });
});
