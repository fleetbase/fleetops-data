...
module('Unit | Serializer | tracking number', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        this.serializer = this.store.serializerFor('tracking-number');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'tracking-number');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'tracking-number', {});
    });

    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'tracking-number', { tracking_number: 'contract-value' });

        assert.strictEqual(record.tracking_number, 'contract-value', 'the attribute survives normalization');
    });

    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('tracking-number', { tracking_number: 'contract-value' });

        assert.strictEqual(record.serialize().tracking_number, 'contract-value');
    });
});
