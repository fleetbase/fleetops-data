...
module('Unit | Serializer | route', function (hooks) {
    setupTest(hooks);

    hooks.beforeEach(function () {
        this.store = this.owner.lookup('service:store');
        this.serializer = this.store.serializerFor('route');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'route');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'route', {});
    });

    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'route', { order_status: 'contract-value' });

        assert.strictEqual(record.order_status, 'contract-value', 'the attribute survives normalization');
    });

    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('route', { order_status: 'contract-value' });

        assert.strictEqual(record.serialize().order_status, 'contract-value');
    });
});
