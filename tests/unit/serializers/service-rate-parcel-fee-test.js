...
module('Unit | Serializer | service rate parcel fee', function (hooks) {
    ...
    hooks.beforeEach(function () {
        ...
        this.serializer = this.store.serializerFor('service-rate-parcel-fee');
    });
    ...
    test('it is a Fleetbase application serializer that embeds records', function (assert) {
        ...
        assertPrimaryKeyIsUuid(assert, this.store, 'service-rate-parcel-fee');
    });

    test('it declares exactly the expected relationship serialization contract', function (assert) {
        assertEmbeddedAttrs(assert, this.store, 'service-rate-parcel-fee', {});
    });

    test('a server payload is normalized onto a record keyed by uuid', function (assert) {
        const record = assertNormalizesUuidAsId(assert, this.store, 'service-rate-parcel-fee', { size: 'contract-value' });

        assert.strictEqual(record.size, 'contract-value', 'the attribute survives normalization');
    });

    test('a record serializes its attributes back onto the wire', function (assert) {
        const record = this.store.createRecord('service-rate-parcel-fee', { size: 'contract-value' });

        assert.strictEqual(record.serialize().size, 'contract-value');
    });
});
