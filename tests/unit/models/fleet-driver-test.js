...
module('Unit | Model | fleet driver', function (hooks) {
    setupTest(hooks);
    ...
    test('relationships target the right models with the right loading strategy', function (assert) {
        assertRelationships(assert, this.store, 'fleet-driver', {
            fleet: { kind: 'belongsTo', type: 'fleet' },
    ...
    test('updated_at renders its formatting getters', function (assert) {
        assertDateGetters(
            assert,
            this.store.createRecord('fleet-driver'),
            'updated_at',
            {
