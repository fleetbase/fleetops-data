...
module('Unit | Model | equipment', function (hooks) {
    setupTest(hooks);
    ...
    test('relationships target the right models with the right loading strategy', function (assert) {
        assertRelationships(assert, this.store, 'equipment', {
    ...
    test('updated_at renders its formatting getters', function (assert) {
        assertDateGetters(
            assert,
            this.store.createRecord('equipment'),
            'updated_at',
            {
    ...
    test('purchased_at renders its formatting getters', function (assert) {
        assertDateGetters(
            assert,
            this.store.createRecord('equipment'),
            'purchased_at',
            {
                purchasedAt: FIXED_DATE_LONG,
                purchasedAtShort: FIXED_DATE_SHORT,
            },
            {
                purchasedAgo: THREE_DAYS_DISTANCE,
            }
        );
    });
});
